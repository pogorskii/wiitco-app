...
export async function GlobalSearchResults({ search }: { search: string }) {
  const [games, movies, televisionShows, animeShows] = await Promise.all([
  ...
  return (
    <div className="bg-white w-full absolute top-12 left-0 z-50">
      <ul>
        {sortedSearchResults.map((e, i) => {
          return (
            <li key={i}>
              <Link className="block py-2" href={e.link}>
