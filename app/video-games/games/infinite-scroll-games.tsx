...
import { fetchGamesByNameInf, fetchGames } from "./actions";
...
export default function InfiniteScrollGames({
  ...
  //   const next = page + 1;
  //   const games = await fetchGamesByNameInf({ page: next, search });
  //   if (games?.length) {
  //     setPage(next);
  ...
  return (
    <>
      {/* <Suspense
        key={search + page + Math.random()}
        fallback={<GamesTableSkeleton />}
      > */}
      {games?.map((game) => (
